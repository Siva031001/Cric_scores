...
/** A legal delivery is one that counts toward the over — not a wide/no-ball, and not a NEW_BATSMAN bookkeeping marker. */
export const isLegalDelivery = (ball) => {
  if (!ball || ball.type === 'NEW_BATSMAN') return false;
  const r = ball.result;
  return typeof r === 'string' && !r.startsWith('WD') && !r.startsWith('NB');
};
 ...
 */
export const currentPartnership = (ballHistory) => {
...
/** Fall of wickets: [{ wicketNumber, score, over }], derived by replaying ballHistory forward. */
export const fallOfWickets = (ballHistory) => {
  const balls = ballHistory ?? [];
  const fow = [];
  let score = 0;
  let wickets = 0;
  for (const b of balls) {
    ...
    if (isWicketResult(b?.result)) {
      wickets += 1;
      fow.push({ wicketNumber: wickets, score, over: b.over != null ? `${b.over}.${(b.ball ?? 0) + 1}` : null });
    }
  }
  return fow;
};
